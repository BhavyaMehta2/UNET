...
class DataNode extends UnetAgent {
  ...
  void processMessage(Message msg) { // Method to process received messages
    if(msg instanceof DatagramNtf && msg.from != 1) { // Check if received message is a datagram notification and not from self
      channelBusy = true // Set channel busy flag
      add new WakerBehavior(1500,  { // Schedule behavior to reset channelBusy flag after 1500ms
        channelBusy = false
      })
    }
    else if (msg instanceof DatagramNtf && msg.protocol == Protocols.INIT) { // Check if received message is INIT protocol
      ...
      add new WakerBehavior(backoff, { // Schedule response after backoff time
        if(!channelBusy){ // Check if channel is not busy
      ...
      if(dataGathered != 2) { // Check if data is gathered
        add new WakerBehavior(tdmaSlotLength*msg.data.size() + senseTime,  { // Schedule CSMA transmission after sense time
          if(!channelBusy){ // Check if channel is not busy
            ...
            log.info 'Backoff due to clash...' // Log backoff due to clash
            add new WakerBehavior(rnd.nextInt(csmaSlotLength-2000-senseTime),  { // Schedule transmission after random backoff time
              phy << new DatagramReq(
                recipient: msg.sender,
                to: msg.from,
                protocol: Protocols.CSMA,
