...
simulate T, {  
  nodes.each { myAddr ->
    def myNode = node "${myAddr}", address: myAddr, location: nodeLocation[myAddr], stack: "$home/etc/setup"

    myNode.startup = {
      if(myAddr!=1)
      {
        def mac = agentForService MAC
        ...
        add new TickerBehavior(5000, {
          ...
          add new WakerBehavior(dataAt, {
              ...
              {  
                phy << new ClearReq()
                phy << new TxFrameReq(to: 1, type: DATA)
              }
              else
              {
                log.info 'Backoff due to clash...'
                add new WakerBehavior(1500, {
                  phy << new ClearReq()
                  phy << new TxFrameReq(to: 1, type: DATA)
                })
              }
